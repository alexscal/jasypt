...
public final class StandardPBEByteEncryptor implements PBEByteCleanablePasswordEncryptor {
    ...
    private static Logger LOGGER = LoggerFactory.getLogger(StandardPBEByteEncryptor.class);
    ...
    private int IVSizeBytes = IV_SIZE_IN_BITS;
     ...
     * an instance of {@link org.jasypt.salt.RandomIVGenerator} will be used.
     * </p>
     *
    ...
    public synchronized void initialize() {
        ...
        if (!this.initialized) {
            ...
            if (this.config != null) {
                ...
                this.ivGenerator = ((this.ivGeneratorSet) || (configIVGenerator == null))?
            ...
            if (this.ivGenerator == null) {
                this.ivGenerator = new RandomIVGenerator();
            }
            ...
            if (this.usingFixedSalt) {
                ...
                // Generate the IV
                byte[] iv = ivGenerator.generateIV(IVSizeBytes);
                LOGGER.debug("Initialising the IV, with size of {} bytes: {}", IVSizeBytes, Arrays.toString(iv));
    ...
    public byte[] encrypt(final byte[] message) 
        ...
        if (LOGGER.isDebugEnabled())
            LOGGER.debug("Original Message to Encrypt: {}", Arrays.toString(message));
        ...
        try {
            ...
            if (this.usingFixedSalt) {
                ...
                if (LOGGER.isDebugEnabled())
                    LOGGER.debug("Salt Generated: {}", Arrays.toString(salt));
                ...
                synchronized (this.encryptCipher) {
                    ...
                    if (LOGGER.isDebugEnabled())
                        LOGGER.debug("Message Encrypted before Adding the IV: {}", Arrays.toString(encryptedMessage));
                }
            ...
            if (ivGenerator.includePlainIVInEncryptionResults()) {
                ...
                if (LOGGER.isDebugEnabled())
                    LOGGER.debug("Encrypted Message with appended IV: {}", Arrays.toString(encryptedMessageWithIV));
            }
            ...
            if (this.saltGenerator.includePlainSaltInEncryptionResults()) {
                ...
                byte[] encryptedMessageWithIVAndSalt = CommonUtils.appendArrays(salt, encryptedMessageWithIV);
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Salt used: {}", Arrays.toString(salt));
                    LOGGER.debug("Encrypted Message with appended IV and Salt: {}", Arrays.toString(encryptedMessageWithIVAndSalt));
                }
    ...
    public byte[] decrypt(final byte[] encryptedMessage) 
        ...
        if (LOGGER.isDebugEnabled())
            LOGGER.debug("Original Message to Decrypt: {}", Arrays.toString(encryptedMessage));
...
//        }
        ...
        try {
            ...
            if (this.saltGenerator.includePlainSaltInEncryptionResults()) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Extracted Salt: {}", Arrays.toString(salt));
                    LOGGER.debug("Encrypted Message after removing the salt: {}", Arrays.toString(encryptedMessageKernel));
                }
            ...
            // Logic for IV
            if (LOGGER.isDebugEnabled())
                LOGGER.debug("Final Encrypted Message with IV: {}", Arrays.toString(encryptedMessageKernel));
            ...
            if (LOGGER.isDebugEnabled())
                LOGGER.debug("Final Encrypted Message to Decrypt: {}", Arrays.toString(finalEncryptedMessage));
            ...
            if (this.usingFixedSalt) {
                ...
                synchronized (this.decryptCipher) {
                    decryptCipher.init(Cipher.DECRYPT_MODE, this.key, parameterSpec);
            ...
            if (LOGGER.isDebugEnabled())
                LOGGER.debug("Final Decrypted Message in Bytes: {}", Arrays.toString(decryptedMessage));
